...
@Composable
fun ERegistrar(viewModel: MainViewModel){

    var nombresE by rememberSaveable { mutableStateOf(("")) }
    var codigoE by rememberSaveable { mutableStateOf(("")) }
    ...
    LazyColumn(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier
        ...
        item {
            ...
            Button(onClick = {
                viewModel.addStudent(Estudiante(codigoE.toInt(), nombresE))
                viewModel.addStudentCourseCrossRef(EstudianteCursoR(codigoE.toInt(), courseId.toInt()))
            }) {
